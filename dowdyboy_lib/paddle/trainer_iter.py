...
class Trainer(object):
    ...
    def _save_checkpoint(self, step, have_val):
        ...
        if step % self.config.save_interval == 0:
            _save_all(self, os.path.join(self.config.out_dir, 'checkpoint', f'iter_{step}'))
        if self.config.save_last:
            _save_all(self, os.path.join(self.config.out_dir, 'checkpoint', f'last_iter_{step}'))
            _del_checkpoint(self, 'last', step)
        if self.config.save_best and step % self.config.val_interval == 0 and have_val:
            ...
            if (self.config.save_best_type == 'min' and best_rec < self.save_best_val) \
                ...
                _save_all(self, os.path.join(self.config.out_dir, 'checkpoint', f'best_iter_{step}'))
    ...
    def set_train_dataloader(self, train_loader, shuffle=True):
        if isinstance(train_loader, list):
            self.train_dataloader = [self._translate_data_loader(loader, shuffle=shuffle) for loader in train_loader]
        else:
